...
namespace AdventOfCode2018.Day10
{
    class Day10
    {
        public void Part1(string[] input)
        {
            ...
            {
                var left = points.Min(x => x.XPosition) - 1;
                var right = points.Max(x => x.XPosition) + 1;
                var top = points.Min(x => x.YPosition) - 1;
                var bottom = points.Max(x => x.YPosition) + 1;
                ...
                if (right - left < 200 && bottom - top < 200)
                {
                    ...
                    for (var y = top; y <= bottom; y++)
                    {
                        ...
                        {
                            output.Append(points.Any(p => p.XPosition == x && p.YPosition == y) ? "#" : ".");
                        }
        ...
        private IList<PointOfLight> GetPoints(string[] input)
        {
            var points = new List<PointOfLight>(input.Length);
            foreach (var line in input)
            {
                var match = Regex.Match(line, @"position=<\s*([^,]*),\s*([^>]*)>\s*velocity=<\s*([^,]*),\s*([^>]*)>");
                points.Add(new PointOfLight
                {
                    XPosition = int.Parse(match.Groups[1].Value),
                    YPosition = int.Parse(match.Groups[2].Value),
        ...
        public static void Run()
        {
            ...
            var exampleInput = @"position=< 9,  1> velocity=< 0,  2>
...
position=<-3,  6> velocity=< 2, -1>".Split(new[] { "\r\n" }, StringSplitOptions.RemoveEmptyEntries);
            var Day10 = new Day10();
            Console.WriteLine("\n###############\n###############\nDay10\n###############\n###############\n");
            Console.WriteLine("\n###############\nPart 1\n###############\n");
            Day10.Part1(exampleInput);
            Day10.Part1(input);
            
            //Console.WriteLine("\n###############\nPart 2\n###############\n");
            //Console.WriteLine(Day10.Part2(exampleInput));
            //Console.WriteLine(Day10.Part2(input));
        }

        private static string GetInput()
        {
            return @"position=<-30580, -10081> velocity=< 3,  1>
