...
1 VRVDQ => 2 FXGW";
        ...
        {
            var need = new Dictionary<string, int> { { "FUEL", 1 } };
            var reactions = Sample.Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries).Select(x => new Reaction(x));


            return need["ORE"].ToString();
        }
    ...
    class Reaction
    {
        ...
        public (string, int) Output { get; set; }

        public Reaction(string recipe)
        {
            var recipeParts = recipe.Split(" => ", 2);
            Inputs = recipeParts[0].Split(", ").ToDictionary(x => x.Split(' ', 2)[1], x => int.Parse(x.Split(' ', 2)[0]));
            var outputParts = recipeParts[1].Split(' ', 2);
            Output = (outputParts[1], int.Parse(outputParts[0]));
        }
    }
}
