...
U62,R66,U55,R34,D71,R55,D58,R83";
        ...
        public new string Part1()
        {
            ...
            foreach (var instruction in lines[0].Split(','))
            {
                ...
                for (var i = 0; i < steps; i++)
                {
                    ...
                    {
                        case 'U':
                            grid.TryAdd((currentPos.X, ++currentPos.Y), true);
                            break;
                        case 'D':
                            grid.TryAdd((currentPos.X, --currentPos.Y), true);
                            break;
            ...
            foreach (var instruction in lines[1].Split(','))
            {
                var direction = instruction[0];
                var steps = int.Parse(instruction.Substring(1));
                for (var i = 0; i < steps; i++)
                {
                    ...
                    {
                        case 'U':
                            if (grid.ContainsKey((currentPos.X, ++currentPos.Y)))
                                intersections.Add(new Point(currentPos.X, currentPos.Y));
                            break;
                        case 'D':
                            if (grid.ContainsKey((currentPos.X, --currentPos.Y)))
                                intersections.Add(new Point(currentPos.X, currentPos.Y));
            ...
            return intersections.Min(intersection => Math.Abs(intersection.X) + Math.Abs(intersection.Y)).ToString();
        }
        ...
        public new string Part2()
        {
            ...
            foreach (var instruction in lines[0].Split(','))
            {
                ...
                for (var i = 0; i < steps; i++)
                {
                    ...
                    {
                        case 'U':
                            grid.TryAdd((currentPos.X, ++currentPos.Y), ++totalSteps);
                            break;
                        case 'D':
                            grid.TryAdd((currentPos.X, --currentPos.Y), ++totalSteps);
                            break;
            ...
            foreach (var instruction in lines[1].Split(','))
            {
                ...
                {
                    totalSteps++;
                    var otherLineSteps = 0;
                    switch (direction)
                    {
                        case 'U':
                            if (grid.TryGetValue((currentPos.X, ++currentPos.Y), out otherLineSteps))
                                intersections.Add(new Point(currentPos.X, currentPos.Y), totalSteps + otherLineSteps);
                            break;
                        case 'D':
                            if (grid.TryGetValue((currentPos.X, --currentPos.Y), out otherLineSteps))
                                intersections.Add(new Point(currentPos.X, currentPos.Y), totalSteps + otherLineSteps);
            ...
            return intersections.Min(i => i.Value).ToString();
        }
    }
}
