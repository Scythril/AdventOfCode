...
namespace AdventOfCode2017
{
    public class Day7
    {
        private readonly Dictionary<String, TreeNode<Prog>> progs = new Dictionary<String, TreeNode<Prog>>();

        public String Part1(String input)
        {
            TreeNode<Prog> treeRoot;
            progs.Clear();
            var lines = input.Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries);
            foreach (var line in lines)
            {
                var tokens = line.Split(' ', StringSplitOptions.RemoveEmptyEntries);
                var prog = new Prog() { Name = tokens[0], Weight = Int32.Parse(tokens[1].Trim(new[] { '(', ')' })) };
                var node = new TreeNode<Prog>(prog);
                progs.Add(prog.Name, node);
            }
            ...
            foreach (var line in lines)
            {
                ...
                if (tokens.Length < 3)
                    continue;
                var children = tokens[3].Split(new[] { ',', ' ' }, StringSplitOptions.RemoveEmptyEntries);
                foreach (var child in children)
                    progs.Remove(child);
            }
            treeRoot = progs.First().Value;
            return treeRoot.Value.Name;
        }

        public int Part2(String input)
        {
            String treeRootName = Part1(input);
            TreeNode<Prog> treeRoot;
            progs.Clear();
            var lines = input.Split(new[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries);
            foreach (var line in lines)
            {
                ...
                var prog = new Prog() { Name = tokens[0], Weight = Int32.Parse(tokens[1].Trim(new[] { '(', ')' })) };
                if (tokens.Length > 2)
                    prog.ChildNames = tokens[3].Split(new[] { ',', ' ' }, StringSplitOptions.RemoveEmptyEntries).ToList();
                var node = new TreeNode<Prog>(prog);
                progs.Add(prog.Name, node);
            }
            treeRoot = progs[treeRootName];
            SetChildren(treeRoot, null);
            foreach (var child in treeRoot.Children)
            {

            }
            CalculateWeights(treeRoot);
            return FindWeightInbalance(treeRoot);
        }

        private int FindWeightInbalance(TreeNode<Prog> prog)
        {
            // weight, numChildren
            Dictionary<int, int> weightVals = new Dictionary<int, int>();
            foreach (var child in prog.Children)
            {
                var weight = child.Value.Weight + child.Value.ChildrenWeight;
                if (weightVals.ContainsKey(weight))
                    weightVals[weight]++;
                else
                    weightVals.Add(weight, 1);
            }
            if (weightVals.Count == 1)
                return 0;
            var oddWeight = weightVals.First(x => x.Value == 1).Key;
            var problemChild = prog.Children.First(x => x.Value.Weight + x.Value.ChildrenWeight == oddWeight);
            var problemChildImbalance = FindWeightInbalance(problemChild);
            if (problemChildImbalance == 0)
                return problemChild.Value.Weight - (oddWeight - weightVals.First(x => x.Key != oddWeight).Key);
            return problemChildImbalance;
        }

        private int CalculateWeights(TreeNode<Prog> prog)
        {
            foreach (var child in prog.Children)
                prog.Value.ChildrenWeight += CalculateWeights(child);
            return prog.Value.ChildrenWeight + prog.Value.Weight;
        }

        private void SetChildren(TreeNode<Prog> prog, TreeNode<Prog> parent)
        {
            if (prog.Value.ChildNames.Count == 0)
                return;
            foreach (var child in prog.Value.ChildNames)
            {
                SetChildren(progs[child], prog);
                prog.AddChild(progs[child]);
            }
        }
...
tmzxl (23)";
            ...
            Console.WriteLine("\n###############\n###############\nDay 7\n###############\n###############\n");
            Console.WriteLine("\n###############\nPart 1\n###############\n");
            Console.WriteLine(day7.Part1(@"pbga (66)
xhth (57)
ebii (61)
havc (66)
ktlj (57)
...
cntj (57)"));
            ...
            Console.WriteLine("\n###############\nPart 2\n###############\n");
            Console.WriteLine(day7.Part2(@"pbga (66)
xhth (57)
ebii (61)
havc (66)
ktlj (57)
...
cntj (57)"));
    ...
    public class TreeNode<T>
    {
        ...
        public readonly List<TreeNode<T>> _children = new List<TreeNode<T>>();
